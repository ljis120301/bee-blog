...
class AnalyticsManager {
  ...
  async recordPageMetrics(postId, sessionId, metrics) {
    const hasCreds = !!(process.env.PB_SERVICE_TOKEN || process.env.PB_ADMIN_TOKEN || process.env.PB_TOKEN || (process.env.PB_ADMIN_EMAIL || process.env.PB_EMAIL) && (process.env.PB_ADMIN_PASSWORD || process.env.PB_PASSWORD));
    if (!hasCreds) {
      // Dev fallback: skip metrics when no PocketBase creds are configured
    ...
    try {
      ...
      const missingCreds = e?.message?.includes('No PocketBase credentials provided');
      const logFn = missingCreds ? console.warn : console.error;
      logFn('Error recording page metrics:', e);
      if (missingCreds) {
        return { updated: false, reason: 'missing_credentials', error: e?.message };
      }
  ...
  async recordPageView(postId, visitorData, request) {
    const hasCreds = !!(
      ...
      process.env.PB_TOKEN ||
      ((process.env.PB_ADMIN_EMAIL || process.env.PB_EMAIL) && (process.env.PB_ADMIN_PASSWORD || process.env.PB_PASSWORD))
    );
    if (!hasCreds) {
      // Dev fallback: skip recording when no PocketBase creds are configured
      return { counted: false, reason: 'missing_credentials' };
    }
    try {
      ...
      return { counted: true, fingerprint: visitorFingerprint };
        ...
        const missingCreds = error?.message?.includes('No PocketBase credentials provided');
        const logFn = missingCreds ? console.warn : console.error;
        logFn('Error recording page view:', error);
        if (missingCreds) {
          return { counted: false, reason: 'missing_credentials', error: error.message };
        }
