...
class AnalyticsManager {
  ...
  async recordPageView(postId, visitorData, request) {
    try {
      ...
      // Skip counting analytics for local/private IPs unless explicitly allowed
      const allowPrivate = process.env.ANALYTICS_COUNT_PRIVATE_IPS === 'true';
      if (!allowPrivate && this.isLocalOrPrivateIP(ip)) {
        return { counted: false, reason: 'private_or_local_ip', fingerprint: visitorFingerprint };
      }
  ...
  async getAnalyticsData(timeRange = '7d') {
    try {
      ...
      let recentSessions = [];
      try {
        const pageViews = await pb.collection('page_views').getList(1, 25, {
          sort: '-created',
          fields: 'id,post_id,ip_address,user_agent,referrer,language,timezone,screen_resolution,session_id,is_authenticated,user_id,created'
        });
        recentSessions = (pageViews?.items || []).map(v => ({
          id: v.id,
          postId: v.post_id,
          ip: v.ip_address,
